...
from .validator import RuleResult, build_default_rules, normalize
from ..config import (
    CONTACTED_FILTER,
    DB_PATH,
    DEFAULT_INPUT_FILE,
    DEFAULT_OUTPUT_FILE,
    INFO_COMPLETED_FILTER,
    PLANT_FILTER,
    WANTED_COLUMNS,
)
from ..db.repository import ColumnRecord, RunRecord, initialize_schema, insert_run, open_connection
...
def run_analysis(
    ...
    mask_selected = (
        df_normalized["Plant"].isin(PLANT_FILTER)
        & df_normalized["Contacted"].str.strip().str.lower().isin(CONTACTED_FILTER)
        & df_normalized["Info completed"].isin(INFO_COMPLETED_FILTER)
    )
