...
from quality_checker import smoke
...
class SmokeTests(unittest.TestCase):
    def test_main_prints_analysis_summary(self) -> None:
        ...
        output = io.StringIO()
        with (
            patch.object(sys, "argv", ["quality-checker-smoke", "input.xlsx"]),
    ...
    def test_module_entrypoint_calls_main(self) -> None:
        ...
        with (
            patch.object(sys, "argv", ["quality_checker.smoke", str(Path("input.xlsx"))]),
            patch("quality_checker.checkers.som.runner.run_analysis", return_value=result),
            redirect_stdout(io.StringIO()),
            warnings.catch_warnings(),
        ):
            warnings.simplefilter("ignore", RuntimeWarning)
            runpy.run_module("quality_checker.smoke", run_name="__main__")

    def test_main_requires_input_argument(self) -> None:
        with patch.object(sys, "argv", ["quality-checker-smoke"]), redirect_stderr(io.StringIO()):
